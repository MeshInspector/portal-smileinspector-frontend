...
import { getCaseStatusColor, getCaseStatusLabel } from "../../utils/case-status.util.ts";
...
const CasePage: React.FC<object> = () => {
  ...
  const { data: caseData, isLoading: isCaseLoading, error: caseError } = useQuery<Case>({
    queryKey: ['case', code],
    queryFn: () => ApiService.getCase(code),
    enabled: !!code,
  })

  const { data: caseLockData, isLoading: isCaseLockLoading, error: caseLockError } = useQuery<CaseLockState>({
    ...
    },
    {
      ...
      render: (value: string, record: CaseColumn) => {
          ...
          return (
            <Space>
              ...
              <Button 
                ...
                disabled={isCaseLockLoading || (record.status && !caseLockData?.canBeUnlocked)}
              >
        ...
        if (record.field === 'Status') {
          return (
            <Space>
              {caseData?.status ? <Tag color={getCaseStatusColor(caseData?.status)}>{value}</Tag> : value}
              <Button
                type="default"
                size="small"
                icon={<EditOutlined />}
