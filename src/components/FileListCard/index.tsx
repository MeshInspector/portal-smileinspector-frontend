...
import { ECaseFileStatus, type CaseFile, type CaseFileList } from '../../types/case-file'
...
const FileListCard: React.FC<FileListCardProps> = ({ caseCode }) => {
  ...
  const { data: filesData, isLoading: isFilesLoading, error: filesError } = useQuery<CaseFileList>({
    queryKey: ['case-files', caseCode, currentPage, pageSize],
    queryFn: () => ApiService.getCaseFiles(caseCode, { page: currentPage, size: pageSize }),
    enabled: !!caseCode,
  })
  ...
  const uploadFilesMutation = useMutation({
    mutationFn: ({ caseCode, files }: { caseCode: string; files: File[] }) => ApiService.uploadCaseFiles(caseCode, files),
    onSuccess: () => {
  ...
  const handleDeleteFiles = async () => {
    ...
    const selectedFiles = filesData?.data.filter(file => selectedFileIds.includes(file.uid)) || []
  ...
  const handleDownloadFiles = async () => {
    ...
    const selectedFiles = filesData?.data.filter(file => selectedFileIds.includes(file.uid)) || []
    const filesToDownload = selectedFiles.filter(file => file.status !== ECaseFileStatus.Pending)
    const filesToSkip = selectedFiles.filter(file => file.status === ECaseFileStatus.Pending)

    if (filesToDownload.length === 0) {
      message.warning(`Cannot download files with '${ECaseFileStatus.Pending}' status`)
      return
    }

    const fileNames = filesToDownload.map(file => file.key).join(', ')
    const skipMessage = filesToSkip.length > 0 ? ` (${filesToSkip.length} files with 'Created' status will be skipped)` : ''

    message.info(`Downloading ${filesToDownload.length} file(s): ${fileNames}${skipMessage}`)

    try {
      for (const file of filesToDownload) {
        await ApiService.downloadFile(caseCode, file.uid)
      }
      message.success(`Successfully initiated download for ${filesToDownload.length} file(s)`)
    } catch (error) {
  ...
  const handleFilePreview = useCallback(async (fileUid: string) => {
    try {
      ...
      img.onerror = () => {
        ...
        setIsPreviewLoading(false)
        setPreviewImageUrl(null)
        notify.error(new Error('Failed to load image preview'), 'Failed to load image preview')
        ...
        if (isImage) {
          return (
            <a
              onClick={async (e) => {
                e.preventDefault()
                await navigate({ from: '/', to: `/cases/${caseCode}/preview/${record.uid}` })
              }}
            >
              {filePath}
            </a>
    ...
    <>
      <div className="filelistcard-panel">
        <div className="filelistcard-header">
          <div className="files-title">Files ({filesData?.totalCount || 0})</div>
          <Space>
            <Button
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key="upload" type="primary" onClick={handleUploadConfirm} disabled={uploadFileList.length === 0} loading={uploadFilesMutation.isPending}>
            Upload
          </Button>
        ]}
      >
        <Upload.Dragger
          ...
          beforeUpload={() => false}
          onChange={({ fileList }) => setUploadFileList(fileList as UploadFile[])}
        >
