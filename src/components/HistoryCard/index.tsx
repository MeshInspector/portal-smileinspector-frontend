...
const HistoryCard: React.FC<HistoryCardProps> = ({ caseCode }) => {
  ...
  const { data: historyData, isLoading: isHistoryLoading, error: historyError } = useQuery({
    ...
    queryFn: () => ApiService.getCaseHistory(caseCode, { page: historyPage, size: historyPageSize }),
    enabled: !!caseCode,
  })
  ...
  const historyColumns = useMemo(() => [
    ...
    { title: 'Action', dataIndex: 'actionDescription', key: 'actionDescription' },
  ], [])
  ...
  return (
    <div className="historycard-panel">
      <div className="historycard-header">
        <div className="history-title">History ({historyData?.totalCount || 0})</div>
      </div>
      {isHistoryLoading ? (
