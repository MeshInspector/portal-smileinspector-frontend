...
apiClient.interceptors.request.use(async (config) => {
  ...
  if (tokenIsExpired) {
    tokens = await cognitoUserPoolsTokenProvider.getTokens({ forceRefresh: true });
  }
...
export class ApiService {
  ...
  static async getCaseFiles(caseCode: string, params: CaseFilesQueryParams = {}): Promise<CaseFileList> {
    try {
      const { page = 1, size = 10 } = params

      const response = await apiClient.get<CaseFileList>(`/v1/cases/${caseCode}/files`, {
        params: {
          page: page - 1,
          size,
        },
  ...
  static async getFileDownloadUrl(caseCode: string, fileUid: string): Promise<FileDownloadUrlResponse> {
    try {
      const response = await apiClient.get<FileDownloadUrlResponse>(`/v1/cases/${caseCode}/files/${fileUid}/download-url`)
      return response.data
    } catch (error) {
      throw new Error(parseError(error))
    }
  ...
  static async downloadFile(caseCode: string, fileUid: string): Promise<void> {
    try {
      const downloadUrlData = await this.getFileDownloadUrl(caseCode, fileUid)
  ...
  static async getCaseComments(caseCode: string, params: CommentQueryParams = {}): Promise<CommentList> {
    try {
      const { page = 1, size = 10, sort } = params

      const response = await apiClient.get<CommentList>(`/v1/cases/${caseCode}/comments`, {
        params: {
          page: page - 1,
          size,
          sort: sort
  ...
  static async getCaseHistory(caseCode: string, params: CaseHistoryQueryParams = {}): Promise<CaseHistoryList> {
    try {
      const { page = 1, size = 50, sort } = params

      const response = await apiClient.get<CaseHistoryList>(`/v1/cases/${caseCode}/history`, {
        params: {
          page: page - 1,
          size,
          sort
  ...
  static async addCaseComment(caseCode: string, comment: CommentInput): Promise<Comment> {
    try {
      const response = await apiClient.post<Comment>(`/v1/cases/${caseCode}/comments`, comment)
      return response.data
    } catch (error) {
      throw new Error(parseError(error))
    }
  ...
  static async deleteCaseComment(caseCode: string, commentUid: string): Promise<void> {
    try {
  ...
  static async getCaseLock(code: string): Promise<CaseLockState> {
    try {
      const response = await apiClient.get<CaseLockState>(`/v1/cases/${code}/lock`)
      return response.data
    } catch (error) {
      throw new Error(parseError(error))
    }
  ...
  static async createUpload(caseCode: string, dto: PresignedUrlsRequestDto): Promise<PresignedUrlResponseDto[]> {
    try {
      const response = await apiClient.post<PresignedUrlResponseDto[]>(`/v1/cases/${caseCode}/uploads/presigned-urls`, dto)
      return response.data
    } catch (error) {
      throw new Error(parseError(error))
    }
  ...
  static async completeUpload(caseCode: string, dto: UploadsCompleteDto): Promise<void> {
    try {
  ...
  static async generatePresignedUrl(caseCode: string, fileName: string): Promise<string> {
    const presignedUrls = await this.createUpload(caseCode, { filePaths: [fileName] })
  ...
  static async uploadCaseFiles(caseCode: string, files: File[]): Promise<void> {
    try {
      await this.lockCase(caseCode)
      const presignedUrls = await this.createUpload(caseCode, { filePaths: files.map(file => file.name) })
      ...
      const uploadedUids: string[] = await Promise.all(
        presignedUrls.map(async (item) => {
          ...
          if (!response.ok) {
            throw new Error(`Failed to upload ${item.filePath}: ${response.status} ${response.statusText}`)
          }
      ...
      await this.completeUpload(caseCode, { uids: uploadedUids })
    } catch (error) {
      throw new Error(parseError(error))
    } finally {
      await this.unlockCase(caseCode)
    }
  }
}
